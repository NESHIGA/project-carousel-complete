...
import { products, services, stats } from "@/lib/catalog";
import { socialLinks } from "@/lib/social";
import hero from "@/assets/tevexxo-hero-new.jpg.asset.json";

export const Route = createFileRoute("/")({
  head: () => ({ meta: [
    { title: "Tevexxo — Software Engineering & Digital Products" },
    { name: "description", content: "Tevexxo builds scalable software and focused digital products for ambitious teams." },
    { property: "og:title", content: "Tevexxo — Build. Ship. Scale." },
    { property: "og:description", content: "Engineering, design and digital products under one roof." },
...
function CollectionPreview({ eyebrow, title, intro, items, to }: { eyebrow: string; title: string; intro: string; items: typeof services; to: "/services" | "/products" }) {
  return (
  ...
  return <main className="pt-18">
    <section className="relative flex min-h-[720px] items-center overflow-hidden border-b border-border">
      ...
      <div className="relative z-10 mx-auto w-full max-w-7xl px-5 py-24 lg:px-8">
        <div className="max-w-3xl"><p className="section-tag"><span className="pulse-dot" /> Next-gen tech studio</p><h1 className="font-display mt-6 text-5xl font-bold leading-[1.05] sm:text-6xl lg:text-8xl">We build the tech <span className="text-gradient">your business runs on.</span></h1><p className="mt-7 max-w-2xl border-l-2 border-accent/40 pl-5 text-lg leading-8 text-muted-foreground">We design and ship software, and partner with ambitious teams to turn complex ideas into working products.</p><div className="mt-9 flex flex-wrap gap-3"><Button asChild size="lg"><Link to="/contact">Start a project <ArrowRight className="size-4" /></Link></Button><Button asChild size="lg" variant="outline"><Link to="/projects">Explore our work</Link></Button></div></div>
        <div className="mt-20 grid max-w-3xl grid-cols-2 gap-8 border-t border-border pt-7 sm:grid-cols-4">{stats.map(([number,label]) => <div key={label}><strong className="font-display text-2xl">{number}</strong><p className="mt-1 text-xs text-muted-foreground">{label}</p></div>)}</div>
      </div>
    </section>
